...
function Get-Python3Command {
    # A WindowsApps python3.exe alias can be discoverable while returning
    # exit code 9009 when Python is not installed. Probe the executable rather
    # than treating command discovery alone as proof that it is usable.
    $python3 = Get-Command python3 -ErrorAction SilentlyContinue
    if ($python3) {
        try {
            $ver = & $python3.Source --version 2>&1
